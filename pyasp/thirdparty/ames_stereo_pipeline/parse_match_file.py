...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Convert an ASP (binary) match file into a text file. Use option -rev to do the reverse operation."
    )
    ...
    parser.add_argument(
        "-rev",
        dest="rev",
        help="Convert a text file into an ASP match file.",
        action="store_true",
    )
    ...
    if not args.rev:
        ...
        try:
            im1_ipb = np.genfromtxt(
                args.infile,
                skip_header=1,
                dtype="float32, float32, int32, int32, float32, float32, float32, int8, uint32, uint32, uint64",
                max_rows=size1,
            )
            im1_ipb = im1_ipb.reshape((size1,))
            im2_ipb = np.genfromtxt(
                args.infile,
                skip_header=1 + int(size1),
                dtype="float32, float32, int32, int32, float32, float32, float32, int8, uint32, uint32, uint64",
                max_rows=size2,
            )
